...
command = """CREATE TABLE IF NOT EXISTS farmer(name TEXT, password TEXT, mobile TEXT, email TEXT)"""
cursor.execute(command)
...
def farmerreg():
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO farmer VALUES ('"+name+"', '"+password+"', '"+mobile+"', '"+email+"')")
        connection.commit()
...
def fertilizer():
    ...
    api_key = "PCOFMB2BIDZOZ627"
    channel_id = "2486069"
    data=requests.get("https://api.thingspeak.com/channels/{}/feeds.json?api_key={}&results=2".format(channel_id, api_key))
    hum=float(data.json()['feeds'][-1]['field1'])
    soil=float(data.json()['feeds'][-1]['field2'])
    ...
    return render_template('fertilizer.html', hum=hum, soil=soil, temp=temp, n=n, p=p, k=k)
...
def Yield():
    if request.method == 'POST':
        ...
        out_1 = forest.predict([[float(state),
                                 ...
                                 float(soilmoisture),
                                 float(area)]])[0]
        yield_res=f"{int(out_1)}  Tons or Kg"
        return render_template('yield.html', prediction=yield_res)
    api_key = "PCOFMB2BIDZOZ627"
    channel_id = "2486069"
    data=requests.get("https://api.thingspeak.com/channels/{}/feeds.json?api_key={}&results=2".format(channel_id, api_key))
    hum=float(data.json()['feeds'][-1]['field1'])
    ...
    p=float(data.json()['feeds'][-1]['field5'])
    k=float(data.json()['feeds'][-1]['field6'])
    return render_template('yield.html', hum=hum, soil=soil, temp=temp, n=n, p=p, k=k)
...
##            return render_template('crop.html', msg="Some thing went wrong, try again")
    ...
    p=float(data.json()['feeds'][-1]['field5'])
    k=float(data.json()['feeds'][-1]['field6'])
    return render_template('crop.html', hum=hum, soil=soil, temp=temp, n=n, p=p, k=k)
...
def tools():
        ...
        if result:
            ...
            return render_template('tools.html', result=result, profile=profile)
        else:
...
def sell_tool():
    if request.method == 'POST':
        ...
        return render_template('tool.html', msg="data uploaded successfully")
...
def aweather():
    if request.method == 'POST':
        try:
            ...
            print(rem)
            return render_template('aweather.html', city=city, temp=temp, time=Time, sky=sky)
        except:
            return render_template('aweather.html', msg="Try again")
    return render_template('aweather.html')
    ...
    if table:
        ...
        for row in rows:
            ...
            if cells:
                ...
                result.append(d[:-1])
    return render_template('amarket.html', result=result)
    ...
    if result:
        ...
        for row in result:
            ...
            if result1:
                ...
                rows.append([row[0], row[1], row[3], "data:image/jpeg;base64,"+row[2], []])
        return render_template('acommunity.html', rows = rows)
    return render_template('acommunity.html', msg="FAQ not found")
